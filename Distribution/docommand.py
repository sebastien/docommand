...
class Command(object):
	...
	def _parseExpression(self, expression):
		...
		while (offset < len(expression)):
			...
			if match:
				...
				self_1187967376_8310=res
				self_1187967376_8310.append(["T", expression[offset:match.start()]])
				if default_argument:
					self_1187967376_8358=res
					self_1187967376_8358.append(["O", argument_number, default_argument_type, default_argument])
				elif True:
					self_1187967376_8351=res
					self_1187967376_8351.append(["A", argument_number, None, None])
				offset = match.end()
			elif True:
				self_1187967376_8324=res
				self_1187967376_8324.append(["T", expression[offset]])
				offset = len(expression)
		return res
	
	def addArgument(self, defaultValue):
		self_1187967376_8429=self.arguments
		self_1187967376_8429.append((defaultValue or ""))
	...
	def getArguments(self, arguments=None):
		...
		res=[]
		for a in arguments:
			self_1187967376_8471=res
			self_1187967376_8471.append(a)
		while (len(res) < len(self.arguments)):
			self_1187967376_8433=res
			self_1187967376_8433.append(self.arguments[(len(res) - 1)])
		return res
	...
	def fill(self, arguments):
		...
		for element in self.parsedExpression:
			if (element[0] == "T"):
				self_1187967376_852=res
				self_1187967376_852.append(element[1])
			elif (element[0] == "A"):
				arg_number=element[1]
				if (len(arguments) <= arg_number):
					raise ERR_MISSING_ARGUMENTS
				elif True:
					self_1187967376_8558=res
					self_1187967376_8558.append(arguments[arg_number])
			elif (element[0] == "O"):
				arg_number=element[1]
				if (len(arguments) <= arg_number):
					if (element[2] == ":"):
						self_1187967376_8575=res
						self_1187967376_8575.append((("`" + element[3]) + "`"))
					elif True:
						self_1187967376_8687=res
						self_1187967376_8687.append(element[3])
				elif True:
					self_1187967376_869=res
					self_1187967376_869.append(arguments[arg_number])
		return "".join(res)
	...
	def parseFile(self, path):
		...
		for line in fd.readlines():
			...
			if (line and (line[0] != "#")):
				...
				self_1187967376_8781=commands
				self_1187967376_8781.append(Command(command_name, command_desc))
		return commands
...
class Interpreter(object):
	def __init__ (self, configuration=None):
		self.parser = Parser()
		self.commands = []
		if configuration is None: configuration = "/Users/sebastien/.docommands"
		self.commands = self.parser.parseFile(configuration)
